...
import { 
  ...
  Footprints, User, Search, ChevronRight 
} from 'lucide-react';
    ...
    <div style={appContainerStyle}>
      <div style={contentAreaStyle}>
        ...
        <div style={aiCardStyle}>
          ...
          <button style={{ background: '#3B82F6', border: 'none', borderRadius: '10px', padding: '8px', cursor: 'pointer' }}>
            ...
            <motion.div key={post.id} whileTap={{ scale: 0.98 }} onClick={() => setSelectedPost(post)} style={postCardContainer}>
              <div style={imageWrapper}>
                ...
                <div style={postOverlay}>
                  {post.hot && <div style={{ background: '#EF4444', color: '#fff', fontSize: '10px', padding: '4px 8px', borderRadius: '6px', width: 'fit-content', marginBottom: '8px', fontWeight: 'bold' }}>🔥 HOT</div>}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px', padding: '0 5px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: '#64748b', fontWeight: '600' }}>{post.author}</span>
                </div>
          ...
          {selectedPost && (
            <>
              ...
              >
                ...
                <div style={drawerStatsRow}>
                  <span>🚶 距离: {selectedPost.stats.distance}</span>
                  <span>⏱ 预计: {selectedPost.stats.time}</span>
                </div>
                <button 
...
const contentAreaStyle: React.CSSProperties = { flex: 1, overflowY: 'auto', position: 'relative', paddingBottom: '100px' };
...
const modalOverlayStyle: React.CSSProperties = { position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 600 };
const drawerStyle: React.CSSProperties = { position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderRadius: '32px 32px 0 0', padding: '20px 25px 40px', zIndex: 700 };
const drawerHandle: React.CSSProperties = { width: '40px', height: '5px', background: '#e2e8f0', margin: '0 auto 20px', borderRadius: '10px' };
const drawerStatsRow: React.CSSProperties = { display: 'flex', justifyContent: 'space-between', margin: '20px 0', background: '#f8fafc', padding: '15px', borderRadius: '15px', fontSize: '14px', fontWeight: 'bold' };
const drawerActionBtn: React.CSSProperties = { width: '100%', background: '#3B82F6', color: '#fff', border: 'none', padding: '16px', borderRadius: '16px', fontWeight: 'bold', cursor: 'pointer' };
